...
function GrnDoc({ grn, onClose }) {
  ...
  /* Merge รายการสินค้าที่เหมือนกัน (same code) → แสดงบรรทัดเดียว
     จำนวน = จำนวนแพ็ครวม, น้ำหนัก/มูลค่า = ผลรวม */
  const mergeMap = {};
  ...
  return (
    <div className="ov" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="md" style={{ width:880, maxHeight:'92vh' }}>
        <div className="md-h">
        ...
        <div className="md-b" style={{ background:'var(--s2)' }}>
          <div style={{ background:'#fff', padding:'28px 36px', width:794, minHeight:1123, maxWidth:'100%', margin:'0 auto', border:'1px solid var(--bd)', fontFamily:'var(--font-sans)', boxShadow:'0 2px 16px rgba(0,0,0,.08)' }}>
            {/* Header */}
            ...
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12.5, marginBottom:16 }}>
              <thead>
                <tr style={{ background:'#1a1826', color:'#fff' }}>
                  <th style={{ padding:'8px 10px', textAlign:'left' }}>รายการที่</th>
                  <th style={{ padding:'8px 10px', textAlign:'left' }}>สินค้า</th>
                  <th style={{ padding:'8px 10px', textAlign:'center' }}>จำนวน</th>
              ...
              <tbody>
                {mergedItems.map((it, i) => (
                  <tr key={i} style={{ borderBottom:'1px solid #eee' }}>
                    ...
                    <td style={{ padding:'8px 10px', textAlign:'center', fontFamily:'var(--font-mono)', fontSize:12, fontWeight:700 }}>{it.packCount}</td>
                    <td style={{ padding:'8px 10px', textAlign:'right', fontWeight:700 }}>{it.totalWeight.toFixed(3)}</td>
                    ...
                    <td style={{ padding:'8px 10px', textAlign:'right', fontWeight:700, color:'var(--gn)' }}>{window.fmtMoney(it.totalValue)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr style={{ background:'#f5f4f0', fontWeight:700 }}>
                  <td colSpan="2" style={{ padding:'8px 10px' }}>รวม {mergedItems.length} รายการ</td>
            ...
            <div style={{ display:'flex', justifyContent:'flex-end' }}>
              <div style={{ width:260, fontSize:12.5 }}>
                <div style={{ display:'flex', justifyContent:'space-between', padding:'5px 0', borderBottom:'1px solid var(--bd)' }}>
                  <span style={{ color:'var(--t2)' }}>จำนวนสินค้า</span><span>{mergedItems.length} รายการ</span>
                </div>
                <div style={{ display:'flex', justifyContent:'space-between', padding:'5px 0', borderBottom:'1px solid var(--bd)' }}>
                  <span style={{ color:'var(--t2)' }}>น้ำหนักรวม</span><span style={{ color:'var(--ac)', fontWeight:700 }}>{window.fmtKg(grn.totalWeight)}</span>
                </div>
                <div style={{ display:'flex', justifyContent:'space-between', padding:'5px 0', borderBottom:'1px solid var(--bd)' }}>
                  <span style={{ color:'var(--t2)' }}>ยอดก่อน VAT</span><span>{window.fmtMoney(grn.totalValue - vat)}</span>
                </div>
                <div style={{ display:'flex', justifyContent:'space-between', padding:'5px 0', borderBottom:'1px solid var(--bd)' }}>
                  <span style={{ color:'var(--t2)' }}>VAT 7%</span><span style={{ color:'var(--pu)' }}>{window.fmtMoney(vat)}</span>
                </div>
  ...
  const doSave = async () => {
    ...
    if (window.SP_API) {
      try {
        ...
        setGrn(newGrn);
        toast('ok', `บันทึก ${grnId} ลง DB เรียบร้อย — ${totalPacks} แพ็ค · ${window.fmtKg(totalW)}`);
