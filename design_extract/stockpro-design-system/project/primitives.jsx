...
const BADGE_MAP = {
  wholesale: { c: 'xb', t: 'Wholesale' }, online: { c: 'xp', t: 'Online' }, sample: { c: 'xx', t: 'Sample' },
...
function StockPill({ stock, min }) {
  ...
  if (stock <= 0) {c = 'spr';t = 'หมดสต็อก';} else
  if (stock < min) {c = 'spa';}
...
function Card({ title, actions, children, style }) {
  return (
    <div className="card" style={style}>
      {(title || actions) && <div className="ch" style={{ color: "rgb(82, 0, 249)", gap: "10px", flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}><span className="ct-t">{title}</span>{actions}</div>}
      {children}
    </div>);
...
Object.assign(window, { Icon, Button, Badge, StockPill, Card, StatCard, Field });